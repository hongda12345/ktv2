...
class gamemanage{
    ...
    function insert(){
        ...
        $data=$mysql->query("insert into game (gname,type) VALUES ('{$gname}','{$type}')")->fetch_all(MYSQLI_ASSOC);
    ...
    function delete(){
        ...
        $mysql=new mysqli('localhost','root','','ktv',3306);
        $mysql->query('set names utf8');
        $data=$mysql->query("delete from game where gid=$ids")->fetch_all(MYSQLI_ASSOC);
        if($mysql->affected_rows){
            echo 'ok';
        }
        echo 'error';
